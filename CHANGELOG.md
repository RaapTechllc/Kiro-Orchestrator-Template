...
- Windows/Git Bash MCP handshake: probe a real Python 3 interpreter, emit UTF-8 JSON-RPC on binary stdio, skip Store/banner lines, pin Git Bash instead of System32/WSL `bash.exe`, translate Git Bash ledger paths for Windows Python I/O, set `isError` on failed orch calls, and omit `cli_stderr` from the envelope.
- MCP JSON envelope `timed_out` flag derived from watchdog markers, plus per-iteration `status`/`timed_out` folded from the ledger so agents do not open `meta.env`.
- Editable copies of Matt Pocock engineering skills under `.cursor/skills/` and `.agents/skills/` (MIT; not the Claude Code marketplace plugin).
